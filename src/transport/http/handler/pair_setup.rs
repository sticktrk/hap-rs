            ...
            match decoded.get(&(Type::State as u8)) {
                Some(method) => match method[0] {
                    ...
                    x if x == StepNumber::SrpVerifyRequest as u8 => {
                        ...
                        let a_proof = decoded.remove(&(Type::Proof as u8)).ok_or(
                            tlv::ErrorContainer::new(
                                StepNumber::SrpVerifyResponse as u8,
                                tlv::Error::Unknown,
                            ),
            ...
            match step {
                Step::Start => match handle_start(self, config).await {
                    ...
                    Err(err) => {
                        ...
                        Err(tlv::ErrorContainer::new(
                            StepNumber::SrpStartResponse as u8,
                            err,
                        ))
                    }
                },
                Step::Verify { a_pub, a_proof } => {
                    match handle_verify(self, &a_pub, &a_proof).await {
                        Ok(res) => {
                            self.unsuccessful_tries = 0;
                            Ok(res)
                        }
                ...
                Step::Exchange { data } => {
                    match handle_exchange(self, config, storage, event_emitter, &data).await {
                        Ok(res) => {
                            self.unsuccessful_tries = 0;
                            Ok(res)
                        }
...
async fn handle_start(
    ...
    let private_key = srp_private_key::<Sha512>(
        b"Pair-Setup",
        &config.lock().await.pin.to_string().as_bytes(),
        &salt,
    ); // x = H(s | H(I | ":" | P))
    ...
    match handler.session {
        ...
        Some(ref mut session) => match session.shared_secret {
            ...
            Some(ref shared_secret) => {
                ...
                let aead =
                    ChaCha20Poly1305::new(GenericArray::from_slice(&encryption_key).as_0_14());
                ...
                let pairing =
                    Pairing::new(pairing_uuid, Permissions::Admin, device_ltpk.to_bytes());
                ...
                let accessory_signature = config
                    .device_ed25519_keypair
                    .sign(&accessory_info.as_slice());
                ...
                let auth_tag = aead.encrypt_in_place_detached(
                    GenericArray::from_slice(&nonce).as_0_14(),
                    &[],
                    &mut encrypted_data,
                )?;
...
mod tests {
    ...
    fn test_verify_client_proof() {
        ...
        let b_proof =
            verify_client_proof::<Sha512>(&b_pub, &a_pub, &a_proof, &salt, &shared_secret, &G_3072)
                .unwrap();
