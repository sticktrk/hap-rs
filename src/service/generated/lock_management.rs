...
use crate::{
    characteristic::{
        ...
        logs::LogsCharacteristic, motion_detected::MotionDetectedCharacteristic,
...
pub struct LockManagementService {
    ...
    pub lock_management_auto_security_timeout:
        Option<LockManagementAutoSecurityTimeoutCharacteristic>,
...
impl LockManagementService {
    ...
    pub fn new(id: u64, accessory_id: u64) -> Self {
        Self {
            ...
            administrator_only_access: Some(AdministratorOnlyAccessCharacteristic::new(
                id + 1 + 0 + 2,
                accessory_id,
            )),
            audio_feedback: Some(AudioFeedbackCharacteristic::new(
                id + 1 + 1 + 2,
                accessory_id,
            )),
            current_door_state: Some(CurrentDoorStateCharacteristic::new(
                id + 1 + 2 + 2,
                accessory_id,
            )),
            lock_management_auto_security_timeout: Some(
                LockManagementAutoSecurityTimeoutCharacteristic::new(id + 1 + 3 + 2, accessory_id),
            ),
            lock_last_known_action: Some(LockLastKnownActionCharacteristic::new(
                id + 1 + 4 + 2,
                accessory_id,
            )),
            logs: Some(LogsCharacteristic::new(id + 1 + 5 + 2, accessory_id)),
            motion_detected: Some(MotionDetectedCharacteristic::new(
                id + 1 + 6 + 2,
                accessory_id,
            )),
            ..Default::default()
...
impl HapService for LockManagementService {
    ...
    fn get_characteristics(&self) -> Vec<&dyn HapCharacteristic> {
        ...
        let mut characteristics: Vec<&dyn HapCharacteristic> =
            vec![&self.lock_control_point, &self.version];
